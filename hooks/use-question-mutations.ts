...
export function useQuestionMutations() {
  ...
  const applyOptimisticUpdates = useCallback(<T extends { _id: string | Id<'questions'> }>(question: T): T => {
    const questionIdStr = typeof question._id === 'string' ? question._id : (question._id as unknown as string)
  ...
  const isOptimisticallyDeleted = useCallback((questionId: string | Id<'questions'>): boolean => {
    const questionIdStr = typeof questionId === 'string' ? questionId : (questionId as unknown as string)
    return optimisticDeletes.has(questionIdStr)
  }, [optimisticDeletes])

  // Helper to filter out optimistically deleted questions
  const filterOptimisticDeletes = useCallback(<T extends { _id: string | Id<'questions'> }>(questions: T[]): T[] => {
  ...
  const applyOptimisticChanges = useCallback(<T extends { _id: string | Id<'questions'> }>(questions: T[]): T[] => {
...
export function useOptimisticDelete() {
  const { optimisticDelete, isOptimisticallyDeleted, filterOptimisticDeletes } = useQuestionMutations()
