...
export class ReviewErrorBoundary extends React.Component<Props, State> {
  ...
  componentDidCatch(error: Error, errorInfo: React.ErrorInfo) {
    ...
    window.dispatchEvent(new CustomEvent('review-error', {
                ...
                <div className="flex-grow">
                  <h2 className="text-lg font-semibold">
                    {isQuestionError ? 'Question Loading Error' :
                     ...
                     isStateError ? 'Review Session Error' :
                     'Something went wrong'}
                  </h2>
                  <p className="text-sm text-muted-foreground mt-1">
                    {this.props.fallbackMessage ||
                     (isQuestionError ? 'Unable to load the review question. This might be temporary.' :
                      isNetworkError ? 'Check your internet connection and try again.' :
                      isStateError ? 'The review session encountered an unexpected state.' :
