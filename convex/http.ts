...
type WebhookEvent = UserCreatedEvent | UserUpdatedEvent | UserDeletedEvent | { type: string; data: unknown };
...
http.route({
  ...
  handler: httpAction(async (ctx, request) => {
    ...
    switch (eventType) {
      case 'user.created':
      case 'user.updated': {
        const { id, email_addresses, first_name, last_name, image_url } = evt.data as UserCreatedEvent['data'];
